...
def barycenter(sta_list):
    ...
    for i in range(1, len(sta_list)):
        ...
        #print('barycenter ',(i-1)*y_mean) 
        x_mean = (sta_list[i].lon + (i-1)*x_mean) / float(i)
        #print(i)
        print('barycenter  ---  --  ', y_mean, ' ' ,x_mean)
...
class ShenStrain:
    ...
    def __init__(self, x=0e0, y=0e0, in_southern_hemisphere=False, station_list=[], **kwargs):
        ...
        #print(len(self.__stalst__))
        #for i in station_list:
            
        #    print(i.__dict__)#3871335.8014126415
        #    print(i.lon)#3871335.8014126415
            #print(i.lat)#3871335.8014126415
            #time.sleep(0.2)
        #print('Xcmp',x)    
        self.__xcmp__   = x # x-ки  сетки
        self.__ycmp__   = y # y-ки  сетки
        self.__in_shemisphere__ = in_southern_hemisphere
        self.__zweights__ = None
        self.__lweights__ = None
        #print('Shein class cx ',self.__xcmp__)
        #print('Shein class cy ',self.__ycmp__)   
    ...
    def filter_sta_wrt_distance(self, d=None):
        ...
        Отфильтруйте список станций экземпляров по граничному расстоянию
            ...
            Эта функция будет вычислять расстояние отсечки:
                 COD = self.__options__['cutoff_dis'] * D
             и отфильтровать все станции в списке __stalst__ экземпляра. новый
             будет возвращен список станций, содержащий только те станции, которые
             меньше (или равно) COD км от центра экземпляра (т.н. точки сетки
             __xcmp__, __ycmp__) точка.
             Если задан входной параметр d, функция будет использовать
             это как D, иначе функция будет использовать self.__options__['d_coef']
             как D (при расчете ХПК).

             Аргументы:
                 d (с плавающей запятой): параметр для вычисления предельного расстояния. Если не
                            при условии, будут использоваться __options__['d_coef']. д
                            следует указывать в км.
            Returns:
            ...
            Note:
                ...
                Возвращаемый список не назначается экземпляру __stalst__.
                 Если хотите, то делайте это вручную.

        """
        #print('grid x ',self.__xcmp__)
        #print(self.__options__['cutoff_dis'])
        #print(self.__options__['d_coef'])
        #time.sleep(0.1)
        if not self.__options__['cutoff_dis']:
            raise ValueError("[ERROR] Cannot filter station list; cutoff_dis is None!")
        # узлы сетки которые прошли проверку по углу 
        cc = Station(lon=self.__xcmp__, lat=self.__ycmp__) ##  передет ситку возвратит объект с пустыми значен кроме сетки
        #print(self.__options__['d_coef'])
        #time.sleep(0.08)
        #print(cc.__dict__)# одна точка сетки на step раз
        if not d: d = self.__options__['d_coef']# on 0 499
        limit = self.__options__['cutoff_dis'] * d # 2.15 * 1    # 2.15 * 2 ...  # 2.15 * 499 мтд         сигма 3 2.14%
        #print(limit)
        ...
        #квадратное расстояние (также известное как удаление квадратных корней).
         ##+ на основе sqrt(Δx^2 + Δy^2) < limit*1e3
         ##+ мы будем использовать (Δx^2 + Δy^2) < limit*limit
#        nlst = [ s for s in self.__stalst__ if s.squared_distance_from(cc) <= 80000 ]
        nlst = [ s for s in self.__stalst__ if s.squared_distance_from(cc) <= limit*limit ]
        #for i in self.__stalst__ :
        #    print(i.squared_distance_from(cc),' <lim ',limit*limit )
        # nlst = ((sta.lon - self.lon)/1e3)^2     +       ((sta.lat - self.lat)/1e3)^2
        #print(nlst)
    ...
    def azimouths(self, other_sta_lst=None):
        """ Azimouth of line containing the instance's centre and each point.
            ...
            are computed as:
            Для вычисления азимутов нам нужны компоненты ΔX и ΔY, которые
             вычисляются как:
            ...
            Args:
                other_sta_lst (list of Station): a list of Station instances.
                    ...
                    other_sta_lst (список станций): список экземпляров станций.
                     Если он не указан, будет использоваться __stalst__.
           ...
           Returns:
                ...
                where each dictionary contains:
                    1. the azimouth value (in radians) as 'az' and
                    2. the index of the station in the other_sta_lst, as 'nr'.
                    (отсортированный) список: этот список состоит из элементов словаря,
                 где каждый словарь содержит:
                     1. значение азимута (в радианах) как «az» и
                     2. индекс станции в other_sta_lst, как 'nr'.

            Note:
                All azimouths will fall in range [0, 2π)
                Все азимуты попадут в диапазон [0, 2π)
        """
        #print(other_sta_lst)
        ...
        for idx, sta in enumerate(stalst):
            #print(sta.name,'',sta.lon, ' - ',self.__xcmp__,' ',sta.lat, ' - ',self.__ycmp__)
            # Разница координат X с каждой точкой сетки и Y с каждой точки сетки по Y
            az = atan2(sta.lon-self.__xcmp__, sta.lat-self.__ycmp__)# atan2(ΔX, ΔY)
            #print(sta.name, ' X',sta.lon,'- grid X ',self.__xcmp__,'Y ',sta.lat,'- y grid',self.__ycmp__)
            azimouths.append({'az': az+int(az<0e0)*(2e0*pi), 'nr': idx})
        #print('ne sort')    
        #print(azimouths)   
        #print('sort')
        azimouths = sorted(azimouths, key=operator.itemgetter('az'))#Группировка пар ключ-значение из словаря
        #переворачивает каждый список задом на перед не сортирует по значению
        #print(azimouths)
        if DEBUG_MODE:
    ...
    def ls_matrices(self, sigma0=1):
        """ Construct Least Squares Matrices (A and b) to be solved for.
            ...
            Матрица A — это «матрица проектирования», а b — вектор наблюдения.
             Эта функция сначала вычислит весовую матрицу (W). 
             формулировка весовой матрицы зависит от опции
             «функция_взвешивания» экземпляра (shen или equal_weights).
             Обратите внимание, что вычисление весовой матрицы фактически выполняется
             через вызов this->make_weight_matrix().
            ...
            Обратите внимание, что функция make_weight_matrix на самом деле не возвращает
             весовая матрица, но ее квадратный корень, он же W = P^(1/2). Этот
             матрица, которую мы будем использовать здесь (нам нужен только квадратный корень из
             матрица весов для умножения на матрицы A и b). Также обратите внимание
             что весовая матрица не масштабируется никаким априорным значением сигма0; следовательно
             масштабирование должно быть сделано здесь (если необходимо).
            ...
            W(i) — (скалярный) квадратный корень из веса станции i для x
                (или восточный) компонент
             W(i+1) — (скалярный) квадратный корень из веса станции i для
                компонент y (или север)
             Для матрицы (собственно вектора) b функция будет иметь вид:
            b(i)   = sta.ve * W(i)
            b(i+1) = sta.vn * W(i+1)
            
            If we solve A\b, we' ll end up with the parameter vector:
            Если мы решим A\b, мы получим вектор параметров:
            [ Ux, Uy, τx, τxy, τy, ω ]**T
        ...
        M = 6
        #print('sigma ',sigma0)
        #print('wes func  ',self.make_weight_matrix())
        ...
        cc  = Station(lon=self.__xcmp__, lat=self.__ycmp__) # bari centr treugolnikov
    ...
    def make_weight_matrix(self):
        """ Construct the square root of weight matrix W <- P^(1/2)
            ...
            Построить квадратный корень из матрицы весов W <- P^(1/2)

             Эта функция построит весовую матрицу, которая будет использоваться для
             оценка деформации (через LSE). Весовая матрица будет сформирована
             согласно опции в __options['weighting_function']__.
             Обратите внимание, что возвращаемая матрица на самом деле представляет собой массив (!!) формы
             (N,1), где N = len(self.__stalst__)*2. Каждый элемент в
             возвращаемая матрица, будет весом для соответствующей станции
             долгота и широта компонента. Например, если __stalst__ = ['dyng', 'ankr', ...],
             тогда W[0] — x-вес dyng, W[1] — y-вес dyng,
             W[2] — x-вес ankr, W[3] — y-вес ankr ....
        ...
        if self.__options__['weighting_function'] == 'shen':
            ...
            #pass
            #print('ves pri equal_weights')
        else:
            raise RuntimeError("[ERROR] Invalid weighting function option")
            #print('ves pri Invalid weighting function option')
        return W
    ...
    def z_weights(self, other_sta_lst=None):
        """ Compute spatial (i.e. azimouthal coverage) weights, accordin to
            ...
            Вычислить пространственные (т. е. азимутальный охват) веса 
             Учитывая список станций, вычислите и верните функцию:
             Z(i) = n*θ(i)/4π
             который используется в качестве весовой функции для оценки деформации в Shen
        
             Веса отдельных станций возвращаются в виде списка в
             порядке, в котором они были переданы в списке other_sta_lst.
             Предполагается, что координаты каждой точки списка и
             координаты __xcmp__, __ycmp__ даны в декартовой RF и
             находятся в метрах.
             Обратите внимание, что (для каждой станции) Station.lon считается «x».
             компонент, а Station.lat считается компонентом 'y'.
    ...
    def compute_theta_angles(self, other_sta_lst=None):
        """ Compute θ angles, aka next minus the previous point.
            ...
            Вычислите углы θ, то есть следующую минус предыдущую точку.
             Составьте список углов «тета»; для каждой точки i угол тета
             разница азимутов предыдущего (i-1) минус следующий
             точка (i+1). Все углы θ будут находиться в диапазоне (0, 2π).
    ...
    def l_weights(self, other_sta_lst=None):
        ...
        """
        ##  Note: d and dri must be in the same units (here km).Примечание: d и dri должны быть в одних и тех же единицах измерения (здесь км).
        # e-(  (корень(X-Xgrid^2+Y-Ygrid^2) /step)^2)
        ...
        if self.__options__['ltype'] == 'gaussian':
            l_i = gaussian # как бы обозначил в переменной имя функции потом по нему вызовим
        elif self.__options__['ltype'] == 'quadratic':
            l_i = quadratic
        else:
            raise RuntimeError("[ERROR] Invalid distance-dependent weighting function")
        #for i in other_sta_lst:
        #    print(i.__dict__)
        stalst = self.__stalst__ if other_sta_lst is None else other_sta_lst
        #for i in other_sta_lst:
        #    print(i.name)
        #print('fdfdfdfdf')
        #  Distances for each point from center in km. (дистанция станции - сетки)^2/1000E0
        dr = [ sqrt((x.lon-self.__xcmp__)*(x.lon-self.__xcmp__)+\
                    (x.lat-self.__ycmp__)*(x.lat-self.__ycmp__))/1000e0 \
                    for x in stalst ]
        #print('dr ',dr[0])
        d = float(self.__options__['d_coef']) # step 1,2,3,4,5,6,7,8 ...
    ...
    def find_optimal_d(self):
        """ Find optimal D coefficient, for distance weighting.
            Найдите оптимальный коэффициент D для взвешивания по расстоянию.
            ...
            Эта функция проверит диапазон [dmin, dstep) с шагом
             dstep (км) для оптимального коэффициента D (он же коэффициент, используемый
             вычислить взвешивание, зависящее от расстояния).
             Оптимальным D является то, для которого:
             int(W) >= int(W_t), где:
             W = Σ{L(i)*Z(i)} для i в 1,2,...,len(станции)*2
             Обратите внимание, что при тестировании отдельных коэффициентов D станции
             используемый список может не совпадать с __stalst__ экземпляра. Это потому,
             для каждого выбранного D __stalst__ фильтруется с использованием:
             self.filter_sta_wrt_distance(D).
            ...
            In summary, the function will:
                - loop through D=[dmin:dmax:dstep]
                    ...
                    * создать новый список станций (newstalst), фильтруя экземпляры
            ...
            Returns:
                ...
                кортеж с элементами:
                 -- список (с плавающей запятой): рассчитанные lweights (т.е. весовые коэффициенты расстояний)
                     с оптимальным D коэфф.
                 -- список (с плавающей запятой): рассчитанные zweights (т.е. пространственные веса)
                     с оптимальным коэффициентом D
                 -- float: оптимальный коэффициент D в км.
            ...
            Возвращаемые списки (lweights и zweights) могут не соответствовать
                 того же размера, что и __stalst__. Они действительно соответствуют списку
                 станций, отфильтрованных с помощью функции:
                 self.filter_sta_wrt_distance(D). Итак, если вы сделаете что-то вроде:
                 lw, zw, D = self.find_optimal_d()
                 valid_sta = self.filter_sta_wrt_distance(D)
                 тогда lw[i] и zw[i] будут весами valid_sta[i] (и
                 не сам.__stalst__[i]).
            Raises:
        ...
        for d in numpy.arange(self.__options__['dmin'], \
                              ...
                              self.__options__['dstep']):
                                                                ...
                                                                #(d) создает массив 0 до 499 с шагом 1
            self.__options__['d_coef'] = d
            
            #time.sleep(0.5)
            #print('self optio d ',self.__options__['d_coef'])
            #print(self.__xcmp__)
            #print(type(d))
            new_sta_lst = self.filter_sta_wrt_distance(d)# передача масива возвращает списки станций 
            #print(len(new_sta_lst))
            #for i in new_sta_lst:
                #print(i)
            #   time.sleep(0.3)
            if len(new_sta_lst) > 3: #если в списке больше 3  почему х его
                #for i in  new_sta_lst:
                #    print(i.__dict__)
    ...
    def beta_angles(self):
        """ Return the β angles (internal angles).
            ...
            aka β = Az(i+1) - Az(i) разница по азимуту следующей минус предыдущей точки
            Составьте список углов β («бета»); для каждой точки угол β
             разница по азимуту следующей минус предыдущей точки
             aka β = Az(i+1) - Az(i) разница по азимуту меняется минус разница точек
        ...
        azimouths = self.azimouths() # отсортированные азимуты в радианах между lon-сеткаlon и lan - сетка lan
        #print('azimouths B',azimouths)
        
        n = len(azimouths)
        #print('UGLy')
        #print(azimouths)
        betas = []
        #print(azimouths)#2*3.14 (маленький угол - большой)
        betas.append(2e0*pi+(azimouths[0]['az'] - azimouths[n-1]['az']))
        
        ####  ### ### '2*3.14 + ugolmin-ugol max',betas И в for 2-1 -3-2 4-3 итд далее так и запихивает в список
        #'2*3.14 + ugolmin-ugol max    2-1                         3-2                4-3
        #[5.98062956108725,           0.16228832611652677, 0.04927039350273532, 0.09099702647307395]
        #[1.440519003517112,          0.0034828520547216524, 0.0011259026626598834, 4.838057548945093]
        #time.sleep(0.5)
        for j in range(0, n-1):
            #print(azimouths[j+1]['az'],'    ', azimouths[j]['az'])
            betas.append(azimouths[j+1]['az'] - azimouths[j]['az'])
        ##  Double-check !! All theta angles must be in the range [0, 2*π)
        if DEBUG_MODE:# ПРоверка если углы больше 0 и меньше или равны 6.28
    ...
    def cmp_strain(self, params_cov=None):
        ''' Compute strain tensor parameters and sigmas
            ...
            Функция также вычислит параметр, соответствующий std.
             значения отклонения (иначе staumax, semax, semin, sazim, sdilat), если
             Матрица дисперсии-ковариации «фундаментальных» параметров имеет вид
             передан 
            ...
            type:
            Матрица дисперсии-ковариации должна быть размером numpy.array
             6х6. Предполагается, что строки, относящиеся к τx, τxy, τy
             параметры, находятся в индексах [2,5), т.е. вар-коварная матрица имеет вид
             тип:
        ...
        col:  0       1       2      3        4         5       
            ...
            Собственно, для выполнения расчета функция собирается сократить
             подматрица params_cov[2:5, 2:5], чтобы все остальные элементы могли
             содержать любые значения.
             Если для параметра функции params_cov установлено значение None, то все значения
             для параметров станд. отклонения, также будет установлено значение «Нет».
            Args:
        ...
        print('tauXX tayXY tauYY  ',x1,x2,x3)
        print(x1*1e9,x2*1e9,x3*1e9)
        print('tauXX+tauYY/2  ',emean)
        print('tauXX-tauYY/2  ',ediff)
        print('TauMax sqrt(tauXY^2+(tauXX-tauYY/2)^2',taumax)
        #time.sleep(1)
        emax  = emean+taumax                ## strain/yr
        emin  = emean-taumax                ## strain/yr
        azim  = -atan2(x2, ediff) / cov / 2.0e0 ## degrees arctan(y,x)
        print('omega',x4)
        print('omega',x4*1e9*0.206e0/3.6e0)
        azim  = 90e0+azim
        dexazim = azim+45e0-180e0
        dilat = x1+x3                       ## strain/yr
        sec_inv = sqrt(x1*x1+2e0*x2*x2+x3*x3)
        print('emax mean+tauMAx',emax)
        print('emin mean-tauMAx',emin)
        print('axim', azim)
        print('dilat XX+YY', dilat)
        print('sec I', sec_inv)
    ...
    def print_details_v2(self, fout, utm_lcm=None):
        ...
        if self.__vcv__ is not None:
            ...
            #print(emax*1e9, semax*1e9, emin*1e9, semin*1e9, taumax*1e9, \
            #staumax*1e9, azim, sazim, dilat*1e9, sdilat*1e9, sec_inv*1e9, ssec_inv*1e9)
            #time.sleep(1)
        else:
            novar = '-'
    ...
    def set_to_barycenter(self):
        ...
        print('funk barycen ', self.__xcmp__)

    def estimate(self):
        """ 
            Оцените основные параметры тензора деформации.
             Эта функция будет (попытаться) оценить фундаментальное значение тензора деформации.
             параметры, также известные как [Ux, Uy, τx, τxy, τy, ω]. Чтобы добиться этого,
             функция выполнит следующее:
            ...
            values:Во время этих шагов функция назначит следующий экземпляр
             ценности
                self.__sigma0__
            ...
            also be assigned:если функция weighting_function указана, то следующие члены будут
             также назначаться:
                self.__zweights__
                self.__lweights__
                self.__stalst__
            ...
            Returns:
                numpy.array (6x1): The least squares solution (or if num. of
                    ...
                    [ Ux, Uy, τx, τxy, τy, ω ]
                    numpy.array (6x1): решение методом наименьших квадратов (или если количество
                     станций — это 3 «точное» решение) для штамма.
                     Массив содержит оценочные значения параметров:
                     [Ux, Uy, τx, τxy, τy, ω]
        ...
        #for i in self.__stalst__:
        #    print(i.name)
        #print('x grid ',self.__xcmp__)    
        #print('wf ',self.__options__['weighting_function'])
        if self.__options__['weighting_function'] == 'shen':
            #print('options d max ',self.__options__['dmax'])
            if not self.__options__['d_coef']:# None          a EGO NET
#                print('net d cof')
                #print(self.__xcmp__)
                self.vprint('[DEBUG] Searching for optimal D parameter.')
                # if 1>=500 или шаг 1 <0
                if self.__options__['dmin'] >= self.__options__['dmax'] or self.__options__['dstep'] < 0:
                    raise RuntimeError
                lwghts, zwghts, d = self.find_optimal_d()
                #print(lwghts)
                #print(lwghts, zwghts, d)
        ...
        VcV  = numpy.dot(A.T, A)
        #print(VcV)
        m, n = A.shape  # отдаст 8 6
        #print('shape m',m,'n ',n,' end')
        
        if m < 6:
            raise RuntimeError('[ERROR] Too few obs to perform LS.')
        elif m == 6:
            #print('m strok matrici == 6 ')
        ...
        #для построения линии регрессии внутри такой функции: Вычислите решение уравнения Ax = b методом наименьших квадратов.
        estim, res, rank, sing_vals = numpy.linalg.lstsq(A, b)
        print('estim ',estim)
        print('suma kvadratov ostatkov evklid norm dly kagd stolbca ',res)
        print('rang matrici ',rank)
        print('singularnoe znachenie matric A ',sing_vals)
        #time.sleep(5)
        # Parameter variance-covariance matrix
        if m > 6:
            #print('m strok bolishe 6')
            #time.sleep(1)
            try:
                ##  A-posteriori std. deviation. res[0] is the sum of residuals; Апостериорная стд. отклонение. res[0] — сумма остатков;
                ##+ squared Euclidean 2-norm for each column in b - a*x, aka + квадрат евклидовой 2-нормы для каждого столбца в b - a*x, иначе
                ##+ u^T * P * u,
