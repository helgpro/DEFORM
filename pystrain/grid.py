...
class Grid:
    """Очень простой класс сетки для использования в проекте StrainTensor.
         Экземпляр Grid имеет ограничения по осям x и y и размеры шагов (т. е. x_min,
         x_max, x_step, y_min, y_max, y_step).
         Это повторяемо; при итерации экземпляр вернет центр
         каждой ячейки, начиная с левого нижнего угла и заканчивая
         в правом верхнем углу. Итерация выполняется построчно
    ...
    def __init__(self, x_min, x_max, x_step, y_min, y_max, y_step, strict_upper_limit=False, upper_limit_epsilon=1e-10):
        ...
        #print('xmin ',self.x_step)
        self.xpts   = int(floor((x_max-x_min) / float(x_step)))# 74-72/0.25
        #print('   self.xpts',self.xpts) #6
        
        self.ypts   = int(floor((y_max-y_min) / float(y_step)))
        #print('   self.ypts',self.ypts)#10
        #print('konstructor GRID')
        if strict_upper_limit:
            print('strict_upper_limit TRUE')
            ...
            print('strict_upper_limit FALSE')
        ...
        #print('x_step   ',x_step)                                 #0.5
        #print('xpts ',self.xpts,' ypts ',self.ypts)
        #print('     upper_limit_epsilon',upper_limit_epsilon)    #1e-10
    ...
    def __iter__(self):
        ...
        self.cxi = 0 # обноляет узлы сетки ?
        #print('  cxi  ',self.cxi)
        self.cyi = 0
        #print('xpts ',self.xpts,' ypts ',self.ypts)# 12 20
        #print('iteratoer')
        return self
    ...
    def xidx2xval(self, idx):
        """Index to value for x-axis.
            ...
            zero).Индекс значения для оси x.
         
             Учитывая индекс (по оси X), верните значение в центре этого
             клетка. Индекс представляет собой номер ячейки (начиная с
             нуль)

            Args:
                idx (int): the index; should be in range (0, self.xpts]
        """
        print('xidx2    ',idx, 'self.xpts ',self.xpts) #ok
        # если текущий шаг сетки меньше константы по x и отличен от 0
        assert idx >= 0 and idx < self.xpts
        # minX + 0.5/2+ 0.5*1.2.3.4.5.6...step
        print('magic ',self.x_min,' + ', 'x_step ',self.x_step,'x)step/2 ',self.x_step/2e0, '+ x_step * idx', self.x_step*float(idx),'=',self.x_min + self.x_step/2e0 + self.x_step*float(idx))
    ...
    def yidx2yval(self, idx):
        ...
        print('magic ',self.y_min,' + ', 'y_step ',self.y_step,'y)step/2 ',self.y_step/2e0, '+ y_step * idx',self.y_step*float(idx),'=',self.y_min + self.y_step/2e0 + self.y_step*float(idx))
        return self.y_min + self.y_step/2e0 + self.y_step*float(idx)

    def next(self):  # Итератор  
        ...
        """
        xi, yi = self.cxi, self.cyi #от 0 до шаг
        #print('nachalo xi = cxi  ',xi)
        # итерация x 0 y0 x1 y0 x2 y0 x3 y0 x0 y1
        #print('nachalo yi = cyi  ',yi)
        # Проверка узел сетки больше констант
        if self.cxi >= self.xpts - 1:# x0 > 6-1 X1 > 6-1
            if self.cyi >= self.ypts - 1: # Y0 > 6-1 Y1 > 6-1
                ...
                if self.cxi == self.xpts - 1 and self.cyi == self.ypts - 1:
                    ...
                    raise StopIteration
            self.cxi  = 0      #добавляет по X след шаг
            self.cyi += 1
        else:
            xi, yi = self.cxi, self.cyi             #добавляет по Y след шаг
            self.cxi += 1
