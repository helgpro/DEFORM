...
#python StrainTensor.py -i ./data/CNRS.vel -r 73/75/39/43 --x-grid-step=0.25 --y-grid-step=0.25
#./gmtstrainplot.sh -jpg -str strain_info.dat -psta -l o outfile              #-str, rot, gdot, gtotaxes, dil, secinv 
#./gmtstrainplot.sh -jpg -str delaunay_info.dat -deltr -psta -l -o outfile 
#./gmtstrainplot.sh -jpg -vhor station_info.dat  -psta -l -o outfile 
'''
...
PAPER_SIZE="30cx30c" : установить пользовательский размер бумаги (ширина x высота) 

vscmagn=20 : величина масштаба горизонтальной стрелки 
VSC=0.05 : шкала горизонтальной скорости 

strscmagn=100 : установить величину масштаба главных осей 
STRSC=0.01 : установить масштаб графика главной оси 

ROTSC=.7 : установить масштаб графика скорости вращения 
ROT_wedge_mag=1 : установить величину вращения 1-клина.
   ...
   # print('oleg ', sta_lst, sta_list_to_degrees)
    ...
    for sta in sta_lst:
        if sta_list_to_degrees:
            #print('sta list ell lon ',sta.lon) # входные кординаты из списка станций
            #print('sta list ell  lan ',sta.lat)
            slon = degrees(sta.lon)# перевод в градусы
            #print(slon) 
            slat = degrees(sta.lat)
        else:
            slon = sta.lon
            slat = sta.lat
        if slon >= xmin and slon <= xmax and slat >= ymin and slat <= ymax:# проверка если координаты больше минимума границ квадрата
            # и меньше максимума границ квадрата новый список
...
def compute__(igrd, sta_list_utm, utm_lcm, fout, fstats, vprint_fun, **dargs):
    ...
    for x, y in igrd:# Gradus
        ...
        #print('clat ',clat<0e0) == False
        #print(E,' ',N)
        #print(len(N))
        #print(len(sta_list_utm))
        sstr = ShenStrain(E, N, clat<0e0, sta_list_utm, **dargs)# экземпляр класса
        #print(sstr.__dict__)
        #print(sstr.__dict__)
        #print('в градусах ', degrees(max(sstr.beta_angles())))
        ...
        #Проверка максимального угла в градусах 
        #print(degrees(max(sstr.beta_angles())))
        #print('provr ',degrees(4.838057548945093))
        if degrees(max(sstr.beta_angles())) <= dargs['max_beta_angle']:# beta po umolch 180
            #print('prohodit StrainTensor.py')
            try:
                
                sstr.estimate()# метод расчета 
...
parser.add_argument('-b', '--barycenter',
    dest='one_tensor',
    action='store_true',
    help='Only estimate one strain tensor, at the region\'s barycentre.Оцените только один тензор деформации в барицентре области.')
...
##########################################################################################################
    ...
    if 'region' in args:
        try:
            lonmin, lonmax, latmin, latmax = [ float(i) for i in args.region.split('/') ] # -r 18/30/32/42 
    ...
    ##+ 1. Compute distance from centre of region to point (lonmax, latmax), aka R Вычислить расстояние от центра региона до точки (lonmax, latmax), иначе R
    ##+ 2. Compute D: User has specified 'D_PARAMETER'? D=2*D_PARAMETER else D=2*D_MAX
    ##+ 3. Compute C: WEIGHTING_FUNCTION='gaussian'? C=R+D*2.15 else C=R+D*10 ВЕСОВАЯ ФУНКЦИЯ Гаусса C=R+D*2.15 иначе C=R+D*10
    ...
    if 'region' in args and not args.method == 'veis' and not args.cut_outoflim_sta:
        ...
        bc =  Station(lon=mean_lon, lat=mean_lat)# вернет объект 'name': None, 'lon': 0.4625122517784973, 'lat': 0.619591884457987, 've': None, 'vn': None, 'se': None, 'sn': None, 'rho': None, 't': None
       
        #print('mean lon lat ',mean_lon, mean_lat)
        #print(bc.__dict__)
        #print('bc obj ',bc.lon)
        endpt = Station(lon=radians(lonmax), lat=radians(latmax))# вернет объект 'name': None, 'lon': 0.4625122517784973, 'lat': 0.619591884457987, 've': None, 'vn': None, 'se': None, 'sn':
        #print(endpt.__dict__)
        #cutoffdis = endpt.h_d(bc)
        cutoffdis = abs(endpt.haversine_distance(bc)/1e3) # barycentre to endpoint (km)Вычисляет дугу в радианах между двумя точками на сфере bc среднее по lon lat в радианах endpt максимумы сетки
        #возвращ модуль с передвинутой точкой на 2 0.0
       
        
        d = 2e0*(args.d_coef if args.d_coef is not None else args.dmax)# так как опция -d не указана берет --dmax*2
       
        cutoffdis += d * (2.15e0 if args.ltype == 'gaussian' else 10e0) # in km по умолчанию == 'gaussian'
        # И того коэфицент d = растояние по дуге кординат на сфере +--dmax*2*2.15e0
        #print(haversine_distance(bc))
        #for ll in sta_list_ell:
         #   print(ll.haversine_distance(bc)/1e3)
        vprint('[DEBUG] Using cut-off distance {:10.3f}km'.format(cutoffdis))
        sta_list_ell = [ s for s in sta_list_ell if s.haversine_distance(bc)/1e3 <= cutoffdis ]# проверка выходят ли данные из списка за число d 
        #for ll in sta_list_ell:
        #    print(ll.__dict__)
        Npst = len(sta_list_ell)
    ...
    #print('floor ',radians(mean_lon))
    #print('floor ',radians(floor(mean_lon)))
    ...
    sta_list_utm = deepcopy(sta_list_ell) # копирование объекта
    ...
    ##  Compute only one Strain Tensor, at the region's barycenter; then exit Оцените только один тензор деформации в барицентре области..
    if args.one_tensor:# если опция -b
    ...
    if args.method == 'shen':  ## Going for Shen algorithm ...
        ...
        if 'region' in args:
            ...
            #####print('region NE ukazan')  
        print(' generate_grid  triangle ')    
        ...
        if args.multiproc_mode:
            ...
            # сетка, данные, средние lon  градусах, фаил для записи, фаил статистики для записи, отладочные сообщения, аргументы
            #print('kol ',len(sta_list_utm))
            #for i in grd:
            #    print('kol ',i)
        ...
        for idx, trng in enumerate(tri.simplices):
            ...
            #print('cx ',cx) # Centr ugla baricentr
            #print('cy ',cy)
